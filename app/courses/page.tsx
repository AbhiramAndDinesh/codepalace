...
export default async function CoursePage() {
  ...
  const usercourses = await getUserCourses(user_id);
  const ids = new Map(usercourses?.map((course) => [course.course_id, true]));
  const courses_ = courses.filter((course) => !ids.has(course.course_id));
